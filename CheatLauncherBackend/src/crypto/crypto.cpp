...
namespace Crypto {
	...
	std::string EncryptAES(std::string message) {
		...
		CryptoPP::AES::Encryption AesEncryption((BYTE*)XOR("passing gas like"), CryptoPP::AES::DEFAULT_KEYLENGTH);
		CryptoPP::CBC_Mode_ExternalCipher::Encryption CbcEncryption(AesEncryption, (byte*)XOR("ahfgcbfhghghtydh"));

		CryptoPP::StreamTransformationFilter stfDecryptor(CbcEncryption, new CryptoPP::StringSink(result), CryptoPP::BlockPaddingSchemeDef::PKCS_PADDING);
	...
	std::string DecryptAES(std::string message, std::string iv, int length) {
		std::string result;
		std::string iv_ = iv.empty() ? std::string(XOR(".?AV<lambda_")) + message.substr(0, 4) : iv;

		CryptoPP::AES::Decryption AesDecryption((BYTE*)XOR("passing gas like"), CryptoPP::AES::DEFAULT_KEYLENGTH);
		CryptoPP::CBC_Mode_ExternalCipher::Decryption CbcDecryption(AesDecryption, (byte*)iv_.c_str());

		if (iv.empty()) message = message.substr(4);

		CryptoPP::StreamTransformationFilter stfDecryptor(CbcDecryption, new CryptoPP::StringSink(result), CryptoPP::BlockPaddingSchemeDef::PKCS_PADDING);

		stfDecryptor.Put(reinterpret_cast<const unsigned char*>(message.c_str()), length == 0 ? message.size() : length);
		stfDecryptor.MessageEnd();
		return result;
	}
	...
	void DecryptArrayAES(uint8_t* encBuffer, int encSize, uint8_t* outBuffer, int outSize, std::string fourKey) {
		CryptoPP::AES::Decryption AesDecryption((BYTE*)XOR("passing gas like"), CryptoPP::AES::DEFAULT_KEYLENGTH);
		CryptoPP::CBC_Mode_ExternalCipher::Decryption CbcDecryption(AesDecryption, (byte*)(std::string(XOR(".?AV<lambda_")) + fourKey).c_str());

		CryptoPP::ArraySink rs(outBuffer, outSize);
		CryptoPP::ArraySource(encBuffer, encSize, true, new StreamTransformationFilter(CbcDecryption, new CryptoPP::Redirector(rs), CryptoPP::BlockPaddingSchemeDef::PKCS_PADDING));

		CryptoPP::HexEncoder(new CryptoPP::ArraySink(outBuffer, outSize)).MessageEnd();
	}
	...
	std::string DecryptResponse(std::string message) {
		return DecryptAES(message.substr(0, 4) + Base64Decode(message.substr(4)));
	}

	uint64_t g_sha512InitialState[XECRYPT_SHA512_DIGEST_SIZE / sizeof(uint64_t)] = {
		0x6A09E667F3BCC908ULL, 0xBB67AE8584CAA73BULL, 0x3C6EF372FE94F82BULL,
		0xA54FF53A5F1D36F1ULL, 0x510E527FADE682D1ULL, 0x9B05688C2B3E6C1FULL,
		0x1F83D9ABFB41BD6BULL, 0x5BE0CD19137E2179ULL
	};
	
	uint64_t g_sha512K[80] = {
	...
	void Sha512Init(XECRYPT_SHA512_STATE* pShaState) {
		memset(pShaState, 0, sizeof(XECRYPT_SHA512_STATE));

		pShaState->m_Count = 0;
		memcpy(&pShaState->m_State, g_sha512InitialState, sizeof(g_sha512InitialState));
	}
	...
	void Sha512Transform(uint64_t* pDigest, uint64_t* pInp) {
		...
		for (i = 0; i < 16; ++i) {
			...
			Temp1 += H + Sigma1(E) + Ch(E, F, G) + g_sha512K[i];
	...
	void Sha512Update(XECRYPT_SHA512_STATE* pShaState, const BYTE* pbInp, DWORD cbInp) {
		...
		if (cbInp >= XECRYPT_SHA512_BLOCK_SIZE) {
			DWORD Blocks = (Index + cbInp) / XECRYPT_SHA512_BLOCK_SIZE;
	...
	void Sha512Final(XECRYPT_SHA512_STATE* pShaState, BYTE* pbOut, DWORD cbOut) {
		...
		DWORD* New = (DWORD*)&pShaState->m_Buffer[XECRYPT_SHA512_BLOCK_SIZE - sizeof(DWORD)];
		DWORD* Input = &Count;
