...
namespace auth::crypto {
	...
	std::string aes_decrypt(std::string message, std::string iv, int length) {
		...
		CryptoPP::CBC_Mode_ExternalCipher::Decryption CbcDecryption(AesDecryption, (uint8_t*)iv.c_str());
...
#define Maj(x,y,z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
	...
	void rc4_modified(uint8_t* pbKey, uint32_t cbKey, uint8_t* pbInpOut, uint32_t cbInpOut, uint32_t startOffset) {
		uint8_t s[256];
		uint8_t k[256];
		uint8_t temp;
		int i, j;
	...
	void sha_transform(uint64_t* pDigest, uint64_t* pInp) {
		uint64_t* W = pInp;
	...
	void sha_update(XECRYPT_SHA512_STATE* pShaState, uint8_t* pbInp, uint32_t cbInp) {
		...
		if (Index) {
			if (Index + cbInp >= XECRYPT_SHA512_BLOCK_SIZE) {
				...
				sha_transform((uint64_t*)pShaState->m_state, (uint64_t*)pShaState->m_buffer);
		...
		if (cbInp >= XECRYPT_SHA512_BLOCK_SIZE) {
			...
			if (IS_ALIGNED_32(pbInp)) {
				for (DWORD i = 0; i < Blocks; ++i) {
					sha_transform((uint64_t*)pShaState->m_state, (uint64_t*)pbInp);
	...
	std::string base64_bytes(uint8_t* payload, uint32_t size) {
		...
		auto get_base_size = [](std::size_t inlen) -> std::size_t {
		...
		std::string out;
		out.resize(get_base_size(size));

		for (std::size_t i = 0, j = 0; i < size; i += 3, j += 4) {
			std::size_t v = payload[i];
			v = i + 1 < size ? v << 8 | payload[i + 1] : v << 8;
			v = i + 2 < size ? v << 8 | payload[i + 2] : v << 8;
