...
namespace menu::hooks {
	...
	bool ros() {
		uint64_t encryption_table = 0;
		uint64_t get_hashing_key = 0;
		bool success = true;

		memory::scan(XOR("ROS_SHA"), [&] (uint64_t address) {
			global::vars::g_crypto_state.m_sha1_init = memory::read_instruction(address, 1, 5);
			global::vars::g_crypto_state.m_sha1_transform = memory::read_instruction(address + 0x1D, 1, 5);
			global::vars::g_crypto_state.m_sha1_update = memory::read_instruction(address + 0x2E, 1, 5);
			global::vars::g_crypto_state.m_sha1_final = memory::read_instruction(address + 0x8A, 1, 5);
			encryption_table = *(uint64_t*)memory::read_instruction(address + 0x50);
			get_hashing_key = memory::read_instruction(address + 0x66, 1, 5);

			return true;
		}, success);

		memory::scan(XOR("ROS_XKE"), [=] (uint64_t address) {
			caller::call<void>(get_hashing_key, encryption_table + 0x20, &global::vars::g_primary_ros_state.m_platform_hashing, 0x10);
			caller::call<void>(memory::read_instruction(address, 1, 5), encryption_table + 0x20, &global::vars::g_primary_ros_state.m_platform_xor, 0x10);
			return true;
		}, success);

		// caller::call<void>(global::vars::g_ros_init_crypto, &global::vars::g_crypto_state);
		strcpy(global::vars::g_primary_ros_state.m_ticket, global::vars::g_rs_info->m_scs_ticket);
		strcpy(global::vars::g_primary_ros_state.m_session_ticket, global::vars::g_rs_info->m_session_ticket);
		strcpy(global::vars::g_primary_ros_state.m_session_key, auth::crypto::base64_bytes(global::vars::g_rs_info->m_session_key, 0x10).c_str());
		return success;
	}
