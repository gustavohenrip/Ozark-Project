...
namespace vehicle::customs::vars {
    ...
    // DRAW_STAT_BAR_WITH_SPLIT
		...
		int IS_OWNERS_ARMORY_TRUCK_INSIDE_BUNKER(int iParam0) {
			if (iParam0 != -1) {
		...
		int IS_OWNERS_ARMORY_AIRCRAFT_INSIDE_DEFUNCT_BASE(int iParam0) {
			if (iParam0 != -1) {
		...
		int IS_OWNERS_HACKER_TRUCK_INSIDE_BUSINESS_HUB(int iParam0) {
			if (iParam0 != -1) {
    ...
    // GENERATE_VEHICLE_STATS_FOR_CURRENT_VEHICLE_INDEX
		void get_vehicle_stats(int iParam0, vehicle_stats* uParam1) {
			float fVar1;
			int iVar2;
			float fVar3;
      
			if (IS_OWNERS_ARMORY_TRUCK_INSIDE_BUNKER(menu::player::get_local_player().m_id) && native::does_entity_exist(menu::script_global(rage::global::_155_customs_base).at(304).at(0).as<Entity>())) {// 1.55 good
			...
			if (IS_OWNERS_ARMORY_AIRCRAFT_INSIDE_DEFUNCT_BASE(menu::player::get_local_player().m_id) && native::does_entity_exist(menu::script_global(rage::global::_155_customs_base).at(308).as<int>())) {// 1.55 good
				if (iParam0 == menu::script_global(rage::global::_155_customs_base).at(308).as<int>()) {// 1.55 good
					return;
				}
			}

			if (IS_OWNERS_HACKER_TRUCK_INSIDE_BUSINESS_HUB(menu::player::get_local_player().m_id) && native::does_entity_exist(menu::script_global(rage::global::_155_customs_base).at(325).as<int>())) {// 1.55 good
		...
		void draw_vehicle_stats(Vehicle Veh) {
			...
			native::get_hud_colour(1, &iHudR, &iHudG, &iHudB, &iHudA);
			iHudA = 76;

			float fVar51 = 0.017f;
			draw_vehicle_stat_rects(global::ui::g_position.x + 0.11f, (Y + fVar51), (0.00078125f * fVar42), (0.00138888f * 6.f), iHudR, iHudG, iHudB, iHudA); // faint bars - w=0.09140625

			fVar51 = (fVar51 + 0.034722f);
			draw_vehicle_stat_rects(global::ui::g_position.x + 0.11f, (Y + fVar51), (0.00078125f * fVar42), (0.00138888f * 6.f), iHudR, iHudG, iHudB, iHudA); // faint bars
			...
			while (iVar58 < 4) {
				fUpdate = (0.00078125f * ((fVar42 / 100.f) * (NewData[iVar58] - CachedData[iVar58])));
				fBarWidth = (0.00078125f * ((fVar42 / 100.f) * CachedData[iVar58]));

				// handles showing what the upgrade will do for u
				if (fUpdate < 0.f) {
					native::get_hud_colour(6, &iHudR, &iHudG, &iHudB, &iHudA);
					draw_vehicle_stat_rects(global::ui::g_position.x + 0.11f, (Y + fVar51), fBarWidth, (0.00138888f * 6.f), iHudR, iHudG, iHudB, iHudA); // less -> red
					fBarWidth = (fBarWidth + fUpdate);
				} else {
					native::get_hud_colour(9, &iHudR, &iHudG, &iHudB, &iHudA);
					draw_vehicle_stat_rects(global::ui::g_position.x + 0.11f, (Y + fVar51), (fBarWidth + fUpdate), (0.00138888f * 6.f), iHudR, iHudG, iHudB, iHudA); // more -> blue
				}

				native::get_hud_colour(1, &iHudR, &iHudG, &iHudB, &iHudA);
				draw_vehicle_stat_rects(global::ui::g_position.x + 0.11f, (Y + fVar51), fBarWidth, (0.00138888f * 6.f), iHudR, iHudG, iHudB, 255); // old value
				fVar51 = (fVar51 + 0.034722f);
				iVar58++;
			}
		}
