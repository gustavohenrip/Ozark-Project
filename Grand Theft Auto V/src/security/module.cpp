...
namespace Security::Module {
	...
	std::vector<Security::Structs::PLDR_DATA_TABLE_ENTRY> GetMappedModules() {
		std::vector<Security::Structs::PLDR_DATA_TABLE_ENTRY> Modules;

		if (ProcessEnvironmentBlock) {
			Security::Structs::PPEB_LDR_DATA Ldr = ProcessEnvironmentBlock->Ldr;
			if (Ldr) {
				...
				if (CurrentEntry) {
					while (CurrentEntry != &Ldr->InLoadOrderModuleList && CurrentEntry != nullptr) {
						Security::Structs::PLDR_DATA_TABLE_ENTRY Current = CONTAINING_RECORD(CurrentEntry, Security::Structs::LDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
						if (Current) {
