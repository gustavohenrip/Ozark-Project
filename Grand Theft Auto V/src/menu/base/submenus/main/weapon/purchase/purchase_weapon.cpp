...
using namespace PurchaseWeaponMenuVars;
...
namespace PurchaseWeaponMenuVars {
	...
	bool PurchaseWeapon(std::string TransactionName, std::string WeaponName, int Price) {
		if (Rage::Engine::ProcessBasketTransaction(-1829708865, 537254313, 4, { { (int)Native::GetHashKey(TransactionName.c_str()), -1, Price, 1, 1 } })) {
			PurchaseWeaponMenu* This = PurchaseWeaponMenu::GetInstance();
	...
	int CalculateTotalPrice() {
		...
		for (auto& Weapon : Global::Arrays::g_Weapons) {
			if (std::find(begin(BlacklistedSubmenuWeapons), end(BlacklistedSubmenuWeapons), Weapon.m_Result) != end(BlacklistedSubmenuWeapons)) {
				std::string TransactionName = GetTransactionNameForWeaponAmmo(Weapon.m_Result);
				if (!TransactionName.empty()) {
					int Price = GetWeaponPrice(TransactionName);
					if (Price > 0) {
						int MaxAmmo = 0;
						if (Native::GetMaxAmmo(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result, &MaxAmmo)) {
							int CurrentAmmo = Native::GetAmmoInPedWeapon(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result);
							if (CurrentAmmo < MaxAmmo) {
								TotalPrice += (MaxAmmo - CurrentAmmo) * Price;
							}
						}
					}
				}
		...
		Utils::GetFiberPool()->Push([] {
					...
					if (!TransactionName.empty()) {
						...
						if (Price > 0) {
							...
							if (Native::GetMaxAmmo(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result, &MaxAmmo)) {
								int CurrentAmmo = Native::GetAmmoInPedWeapon(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result);
								if (CurrentAmmo < MaxAmmo) {
									Weapons.push_back({ (int)Native::GetHashKey(TransactionName.c_str()), -1, Price, 1, MaxAmmo - CurrentAmmo });
									Native::AddAmmoToPed(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result, MaxAmmo - CurrentAmmo);
								}
							}
						}
					}
				...
				if (!Native::HasPedGotWeapon(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result, false)) {
					...
					if (!TransactionName.empty()) {
						...
						if (Price > 0 || Weapon.m_Result == 0x7846a318) {
							...
							Native::GiveWeaponToPed(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result, 1, false, true);
						}
					}
				}
			}
			...
			if (Weapons.size() > 71) {
				while (Weapons.size()) {
					...
					Rage::Engine::ProcessBasketTransaction(-1829708865, 537254313, 4, Current);
				}
			} else Rage::Engine::ProcessBasketTransaction(-1829708865, 537254313, 4, Weapons);

			PurchaseWeaponMenu::GetInstance()->UpdateOnce();
		});
	}

	void Handle(uint32_t Weapon, std::string TransactionName, std::string Name, int Price) {
		if (Price != -1) {
			if (std::find(begin(BlacklistedSubmenuWeapons), end(BlacklistedSubmenuWeapons), Weapon) != end(BlacklistedSubmenuWeapons)) {
				if (Scripts::func_882((int)Weapon)) {
					return;
				}
			}

			if (PurchaseWeapon(TransactionName, Name, Price)) {
				Native::GiveWeaponToPed(Menu::GetLocalPlayer().m_Ped, Weapon, 1, false, true);
				PurchaseWeaponMenu::GetInstance()->UpdateOnce();
			}
...
void PurchaseWeaponMenu::Init() {
	SetName("Purchase Weapon");
	...
	m_Vars.m_Button = addOption(ButtonOption("Purchase All Weapons")
		.addTranslation().addHotkey()
		.addOnClick(PurchaseAllWeapons));

	addOption(BreakOption("Weapons")
		.addTranslation());
}

void PurchaseWeaponMenu::Update() {}

/*Called once on submenu open*/
void PurchaseWeaponMenu::UpdateOnce() {
	m_Vars.m_Button->setTooltipInternal(getString("Total price: $") + std::to_string(CalculateTotalPrice()));

	ClearOptionsOffset(2);

	for (int i = 0; i < NUMOF(Global::Arrays::g_Weapons); i++) {
		auto Weapon = Global::Arrays::g_Weapons[i];
		if (std::find(begin(BlacklistedWeapons), end(BlacklistedWeapons), Weapon.m_Result) != end(BlacklistedWeapons)) {
			continue;
		}
			...
			if (!TransactionName.empty()) {
				...
				if (Price > 0) {
					addOption(ButtonOption(Weapon.m_Name.Get())
						.addOnClick([=] { Handle(Weapon.m_Result, TransactionName, Global::Arrays::g_Weapons[i].m_Name.Get(), Price); })
						.addSideText(Scripts::func_882((int)Weapon.m_Result) ? "FULL" : "$" + std::to_string(Price)));
				}
			}
			...
			if (!Native::HasPedGotWeapon(Menu::GetLocalPlayer().m_Ped, Weapon.m_Result, false)) {
				...
				if (!TransactionName.empty()) {
					...
					if (Price > 0 || Weapon.m_Result == 0x7846a318) {
						addOption(ButtonOption(Weapon.m_Name.Get())
							.addOnClick([=] { Handle(Weapon.m_Result, TransactionName, Global::Arrays::g_Weapons[i].m_Name.Get(), Price); })
							.addSideText(Price == 0 ? getString("FREE") : "$" + std::to_string(Price)));
					}
				}
...
/*Called always*/
void PurchaseWeaponMenu::FeatureUpdate() {}

PurchaseWeaponMenu* _instance;
PurchaseWeaponMenu* PurchaseWeaponMenu::GetInstance() {
	if (_instance == nullptr) {
		_instance = new PurchaseWeaponMenu();
		_instance->Init();
...
PurchaseWeaponMenu::~PurchaseWeaponMenu() { delete _instance; }