...
namespace Auth::Crypto {
	...
	std::string DecryptAES(std::string message, std::string iv, int length) {
		std::string result;
		std::string iv_ = iv.empty() ? std::string(".?AV<lambda_"_Protect) + message.substr(0, 4) : iv;

		CryptoPP::AES::Decryption AesDecryption((BYTE*)"passing gas like"_Protect, CryptoPP::AES::DEFAULT_KEYLENGTH);
		CryptoPP::CBC_Mode_ExternalCipher::Decryption CbcDecryption(AesDecryption, (uint8_t*)iv_.c_str());
